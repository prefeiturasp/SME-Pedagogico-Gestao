...
class StudentPollPortugueseCard extends Component {
  ...
  showOneHideAll(element) {
    ...
    var all_col = document.getElementsByClassName(
      "text-center border poll-select-container "
    );
    ...
    for (var j = 0; j < columnFiltered.length; j++) {
      document.getElementById(columnFiltered[j] + "_head").style.display =
        "none";
    }

    all_col = document.getElementsByClassName(
      "text-center border poll-select-container " + element
    );
  ...
  render() {
    const isForcaAno3 =
      Number(this.props.poll.selectedFilter?.schoolYear) === 2025 &&
      Number(this.props.poll.pollYear) >= 4 &&
      Number(this.props.poll.pollYear) <= 9;
    ...
    if (
      ...
      rendererStudentPollPortuguese = this.props.students.map((student) =>
        ativoBimestre(student, bimestre) ? (
          <StudentPollPortuguese3A
            key={student.sequenceNumber + student.studentCodeEol}
            student={student}
            updatePollStudent={this.props.updatePollStudent}
            editLock1b={this.props.editLock1b}
            editLock2b={this.props.editLock2b}
            editLock3b={this.props.editLock3b}
            editLock4b={this.props.editLock4b}
          />
        ) : null
      );
    } else if (
      ...
      rendererStudentPollPortuguese = this.props.students.map((student) =>
        ativoBimestre(student, bimestre) ? (
          <StudentPollPortuguese3ANivelHipotese
            anoSelecionado={this.props.poll.selectedFilter.schoolYear}
            key={student.sequenceNumber + student.studentCodeEol}
            student={student}
            updatePollStudent={this.props.updatePollStudent}
            editLock1b={this.props.editLock1b}
    ...
    return (
      <div>
        ...
        >
          <thead>
            <tr>
              <th rowSpan="2" className="align-middle border text-color-purple">
                <div className="ml-2">Sondagem</div>
              </th>
              <th
                colSpan="2"
                id="1bim_col_head"
              ...
              >
                <span style={pStyle}>
                  <img
                    src="./img/icon_pt_DADADA.svg"
                    alt="seta esquerda inativa"
                    style={{ height: 20 }}
                  />
                </span>
                <b className="p-4">1° Bimestre</b>
                <span
                  value="2bim_col"
              ...
              >
                ...
                <b className="p-4">4° Bimestre</b>
                <span style={pStyle}>
                  <img
                    src="./img/icon_2_pt_DADADA.svg"
                    alt="seta direita inativa"
                    style={{ height: 20 }}
                  />
                </span>
            ...
            <tr>
              <th className="text-center border poll-select-container 1bim_col">
                <small className="text-muted">Escrita</small>
              </th>
              <th className="text-center border poll-select-container 1bim_col">
          ...
          <tbody>{rendererStudentPollPortuguese}</tbody>
        </table>
