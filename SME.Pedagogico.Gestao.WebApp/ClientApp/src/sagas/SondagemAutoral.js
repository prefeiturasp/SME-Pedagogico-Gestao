...
function* ListarAlunosAutoralMat({ filtro }) {
    try {
        ...
        yield put({ type: Autoral.types.SETAR_ALUNOS_AUTORAL_MATEMATICA, listaAlunosAutoralMatematica });
    } catch (error) {
...
function listarAlunosMatApi(filtro) {
    var url = `/api/SondagemAutoral/Matematica`; //Passar filtros na url
    return fetch(url, {
        method: "get",
        headers: { "Content-Type": "application/json" }
    }).then(response => response.json());
...
function* SalvaSondagemAutoralMat({ alunos }) {
    try {
        yield put({ type: Autoral.types.SET_POLL_DATA_SAVED_STATE });
